...
def check_wifi_status():
    clear_terminal()
    print_header()
    print(COLOR_GREEN + "[*] Vérification du statut du Wifi..." + COLOR_RESET)

    try:
        output = subprocess.check_output(["nmcli", "radio", "wifi"])
        wifi_status = output.decode("utf-8").strip()
        if wifi_status == "enabled":
            print(COLOR_GREEN + "[+] Wifi est activé." + COLOR_RESET)
        else:
            print(COLOR_RED + "[-] Wifi est désactivé. Veuillez l'activer pour utiliser ce script." + COLOR_RESET)
            sys.exit(1)
    except subprocess.CalledProcessError:
        print(COLOR_RED + "[-] Erreur lors de la vérification du statut Wifi. Veuillez vérifier manuellement." + COLOR_RESET)

    input(COLOR_YELLOW + "Appuyez sur Enter pour continuer..." + COLOR_RESET)
